...
Structs are represented linearly in memory such that
```
  struct {a: u8, b: u8, c: u8}
```
 is identical to
```
struct {fido: u8, jimbo: struct {size: u8, age: u8}}
```
...
Closures are syntax sugar for a value of type
```
record { self_type :0 Type(_), call: (self: self_type, args...) -> R, self: self_type }
```
where the size of the context is usually infered.
The type of closures can be abreviated as `(args...) => R`.
